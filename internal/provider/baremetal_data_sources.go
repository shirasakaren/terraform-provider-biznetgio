	...
	resp.Schema = schema.Schema{
		...
		Attributes: map[string]schema.Attribute{
			"config": schema.StringAttribute{
				Computed:            true,
				Sensitive:           true,
				MarkdownDescription: "Konten config OpenVPN kalau ada di response (alias config/config_file/ovpn/content).",
			},
			"raw": schema.StringAttribute{
				Sensitive:           true,
				Computed:            true,
...
func (d *BaremetalOpenVPNDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	if req.ProviderData == nil {
		return
	}
	client, ok := req.ProviderData.(*biznetgio.Client)
...
func (d *BaremetalOpenVPNDataSource) Read(ctx context.Context, _ datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data BaremetalOpenVPNDataSourceModel

	out, err := d.client.Baremetal().OpenVPN(ctx)
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to get baremetal openvpn: %s", err))
		return
	}
	data.Config = types.StringValue(aliasStr(out, "config", "config_file", "ovpn", "content", "data"))
