...
func (d *NeoliteProOSListDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	if req.ProviderData == nil {
		return
	}
	client, ok := req.ProviderData.(*biznetgio.Client)
...
func (d *NeoliteProOSListDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data NeoliteProOSListDataSourceModel
	resp.Diagnostics.Append(req.Config.Get(ctx, &data)...)
	if resp.Diagnostics.HasError() {
		return
	}

	oss, err := d.client.NeolitePro().ProductOSList(ctx, data.ProductID.ValueInt64())
	if err != nil {
...
func (d *NeoliteProIPAvailabilityDataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	if req.ProviderData == nil {
		return
	}
	client, ok := req.ProviderData.(*biznetgio.Client)
...
func (d *NeoliteProIPAvailabilityDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data NeoliteProIPAvailabilityDataSourceModel
	resp.Diagnostics.Append(req.Config.Get(ctx, &data)...)
	if resp.Diagnostics.HasError() {
		return
	}

	out, err := d.client.NeolitePro().ProductIPAvailability(ctx, data.ProductID.ValueInt64())
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to get neolite pro ip availability: %s", err))
		return
	}

	data.ID = types.StringValue(fmt.Sprintf("neolite-pro-ip-availability-%d", data.ProductID.ValueInt64()))
