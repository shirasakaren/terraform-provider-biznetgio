...
func (d *ObjectStorageCredentialsDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	var models []ObjectStorageCredentialModel
	for _, it := range items {
		m := ObjectStorageCredentialModel{
			AccessKey: objDSString(it, "accessKey", "access_key", "accesskey"),
		}
		if v, ok := objMapBool(it, "active"); ok {
			m.Active = types.BoolValue(v)
		}
		models = append(models, m)
	}

	data.Credentials, resp.Diagnostics = types.ListValueFrom(ctx, types.ObjectType{AttrTypes: ObjectStorageCredentialModel{}.AttributeTypes()}, models)
	if resp.Diagnostics.HasError() {
		return
	}
	resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
...
func objParseAccountID(v string) (int64, error) {
	if v == "" {
		return 0, fmt.Errorf("account_id kosong")
	}
	id, err := strconv.ParseInt(v, 10, 64)
	if err != nil {
		return 0, fmt.Errorf("account_id %q bukan angka", v)
	}
	return id, nil
}

func objDSString(v map[string]any, keys ...string) types.String {
	if s, ok := objMapString(v, keys...); ok {
